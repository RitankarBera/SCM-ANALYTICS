...
generate_data.py
-----------------
Generates realistic, SAP Business One (SAP B1) style raw data dumps for the
Innovel Energy Services Pvt Ltd Procurement Analytics Project.

Simulates 12 months of transactional data (broader than the 2-month internship
window, so trend/monthly dashboards have enough history to be meaningful --
this is a documented assumption, see README).

Run:
    python3 generate_data.py

Outputs CSV files into ../data/ that mirror typical SAP B1 table exports:
    Vendor_Master.csv           (~ SAP B1 OCRD)
    Material_Master.csv         (~ SAP B1 OITM)
    Department_Project_Master.csv
    Warehouse_Master.csv        (~ SAP B1 OWHS)
    PR_Data.csv                 (~ SAP B1 OPRQ/PRQ1)
    PO_Data.csv                 (~ SAP B1 OPOR/POR1)
    GRN_Data.csv                (~ SAP B1 OPDN/PDN1 - Goods Receipt PO)
    Invoice_Data.csv            (~ SAP B1 OPCH/PCH1 - A/P Invoice)
    Inventory_Data.csv          (~ SAP B1 OITW - warehouse stock snapshot)
    Price_History.csv           (derived - material rate trend over time)
"""
...
# ---------------------------------------------------------------------------
# 4b. COST OPTIMIZATION CALIBRATION
# The project's headline outcome is a 5-7% procurement cost saving per
# vertical (negotiated/blanket rates vs. the material master's standard/
# benchmark price), achieved through vendor rate comparison and consolidated
# ordering enabled by this analytics project. Raw random pricing above can
# drift outside that band over 12 months of simulated market movement, so we
# calibrate each vertical's PO/Invoice/Price-History pricing to land the
# *realised* savings between 5% and 7%, matching the actual project outcome.
# ---------------------------------------------------------------------------
