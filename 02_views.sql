-- Analytical views over the raw SAP B1-style tables. Power BI (or the
-- dashboard in this project) connects to these views only, not the raw
-- tables, so refreshing the source data and hitting Refresh updates every
-- report from the same numbers.
