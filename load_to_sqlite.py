...
load_to_sqlite.py
------------------
Simulates the SAP B1 database layer: loads the raw CSV data dumps
(/data) into a local SQLite database (procurement.db), then applies the
schema and views defined in /sql.

In the real project this step is replaced by a live connection to the
SAP B1 SQL Server / HANA database -- Power BI (or this dashboard) always
queries the VIEWS, never the raw tables, so re-pointing this script at
the live DB is a drop-in swap.
